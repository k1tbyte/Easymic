...
LRESULT CALLBACK OnHotkeyPressed(int code, WPARAM wParam, LPARAM lParam)
{
...
void InitHotkey()
{
    if (config.isMouseHotkeyMode && config.mouseHotkeyIndex != 0) {
        hMouseHook = SetWindowsHookEx(WH_MOUSE_LL, OnHotkeyPressed, nullptr, 0);
        return;
    }
...
void SwitchMicState()
{
    ...
    if(isMuted)
    {
        ...
        PlaySound(MAKEINTRESOURCE(IDR_UNMUTE),hInst,
    ...
    else
    {
        ...
        PlaySound(MAKEINTRESOURCE(IDR_MUTE),hInst,
                  SND_ASYNC | SND_RESOURCE);
